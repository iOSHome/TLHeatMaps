...
- (void)calculatePixelColors
{
    ...
    TLMatrix *matrix = [[TLMatrix alloc]initWithColumns:CGRectGetWidth(self.bounds) lines:CGRectGetHeight(self.bounds)];
    for (NSInteger x = 0; x < CGRectGetWidth(self.bounds); x ++) {
        for (NSInteger y = 0; y < CGRectGetHeight(self.bounds); y ++) {
...
{
    TLMatrix *densityMatrix = [[TLMatrix alloc]initWithColumns:CGRectGetWidth(self.bounds) lines:CGRectGetHeight(self.bounds)];
